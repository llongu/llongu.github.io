...
var MouseDirection = function (element, opts) {
    ...
    $element.on('mouseenter', function (e) {//判断进入方向 上0  右1 下2 左3
        var r = calculate(e);
		console.log(r)
		if(r==0){$element.find('.content .tops').css({"top":"100%","transition":".5s"});}
		if(r==2){$element.find('.content .bottoms').css({"bottom":"100%","transition":".5s"});}
		
		
		if(r==3){$element.find('.content .lefts').css({"left":"150%","transition":".5s"});}
		if(r==1){$element.find('.content .rights').css({"right":"150%","transition":".5s"});}
	
        opts.enter($element, dirs[r]);
    }).on('mouseleave', function (e) {//判断出去方向
        var r = calculate(e);
        opts.leave($element, dirs[r]);
		if(r==0){$element.find('.content .tops').css({"top":"-50%","transition":"0s"});}
		if(r==2){$element.find('.content .bottoms').css({"bottom":"-50%","transition":"0s"});}
		
		
		if(r==3){$element.find('.content .lefts').css({"left":"-100%","transition":"0s"});}
		if(r==1){$element.find('.content .rights').css({"right":"-100%","transition":"0s"});}
		console.log(r)
