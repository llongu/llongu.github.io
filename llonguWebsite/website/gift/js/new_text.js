...
/*var lists=$(".new_right_picture li");
...
function moves(){
		...
		if(a==0){$(".new_right_picture").css({"marginTop":"0","transition":"0s"});setTimeout(moves,0)}
		}else{
		$(".new_right_picture").css({"marginTop":-(201*a)+"px","transition":".8s"})
		}
...
function nexts(){
	...
	i++;
	if(i>3){i=0;}
	if(i==0){$(".new_right_picture").css({"transition":"0s" });setTimeout(nexts,0)
