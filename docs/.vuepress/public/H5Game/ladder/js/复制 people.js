...
//		console.log((game.canvas.height / 2.5) - (this.h / 2)+"=>最大跳跃高度")
	...
	//高度变化  3s 过度时间不变化 速度加快 
	var h=1200;//canvas.height
	var min=3 //3s;
	var speed=(h/2)/min;//高度变化==>递增
	y=canvas.height;//起始位置
	var timenum=0;
	var timespeed=speed/(min*60)/16;//阀值 16为帧数
	setinterval(function(){
		if(y>=canvas.height/2 || timenum>=speed){
			y+=timenum;
			timenum+=timespeed;
		}else{
			y-=timenum;
