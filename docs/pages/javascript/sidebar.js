module.exports ={
    '/pages/javascript/' : [
            { title:'小技巧',path:'/pages/javascript/tips.md'},
            { title:'this',path:'/pages/javascript/this.md'},
            { title:'bind 实现',path:'/pages/javascript/bind.md'},
            { title:'closure 闭包',path:'/pages/javascript/closure.md'},
            { title:'call apply',path:'/pages/javascript/callApply.md'},
            { title:'prototype 原型 继承',path:'/pages/javascript/prototype.md'},
            { title:'instanceOf',path:'/pages/javascript/instanceOf.md'},
            { title:'new',path:'/pages/javascript/new.md'},
            
            { title:'执行上下文AO,VO,GO',path:'/pages/javascript/executionContext.md'},
            { title: 'eventLoop', path: '/pages/javascript/eventLoop.md' },
            { title:'eventBus',path:'/pages/javascript/eventBus.md'},

            { title:'栈和堆',path:'/pages/javascript/stack.md'},
            { title:'栈和队列',path:'/pages/javascript/queque.md'},
            { title:'Set WeakSet Map WekMap',path:'/pages/javascript/map.md'},

            { title:'ES5 对象的扩展、密封、冻结',path:'/pages/javascript/es5Object.md'},
            { title:'函数，形参，实参的 length',path:'/pages/javascript/arguments.md'},
            { title:'自执行函数表达式与函数声明的调用',path:'/pages/javascript/callFunction.md'},
            { title:'eavl,new Function,setTimeout',path:'/pages/javascript/eavl.md'},
            { title:'{} 花括号的二义性',path:'/pages/javascript/brackets.md'},
            { title:'++运算符',path:'/pages/javascript/add.md'},

            { title: '设计模式', path: '/pages/javascript/designPattern.md' },

            { title: 'redux 简易版', path: '/pages/javascript/redux.md' },
            { title: 'promise 简易版', path: '/pages/javascript/promise.md' },
            { title: 'proxy 使用', path: '/pages/javascript/proxy.md' },
            { title: 'Reflect 反射', path: '/pages/javascript/Reflect.md' },
            { title: 'machine 队列', path: '/pages/javascript/machine.md' },
            { title: '路由模式实现', path: '/pages/javascript/router.md' },
            { title:'值类型和引用类型及深浅拷贝',path:'/pages/javascript/deepClone.md'},
            { title: '防抖节流', path: '/pages/javascript/throttle.md' },
            { title: 'js沙箱环境', path: '/pages/javascript/sendBox.md' },
            { title: 'createElement', path: '/pages/javascript/createElement.md' },
            { title: 'webComponents', path: '/pages/javascript/webComponents.md' },
            { title: 'indexeddb', path: '/pages/javascript/indexeddb.md' },

            { title: '正则', path: '/pages/javascript/expression.md' },
            
      ]
}
  